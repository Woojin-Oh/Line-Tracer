...
# Generate given tracks
def track1():
    # Generate Tracks
    track_x = []
    track_y = []

    xs, ys = line_generator(x0=3.0, y0=3.0, x1=4.5, y1=8.0, mode=(1, 1))
    track_x.extend(xs)
    track_y.extend(ys)

    xs, ys = line_generator(x0=4.5, y0=8.0, x1=6.0, y1=10.0, mode=(2, 2))
    track_x.extend(xs)
    track_y.extend(ys)
    
    xs, ys = line_generator(x0=6.0, y0=10.0, x1=12.0, y1=7.0, mode=(2, 1))
    track_x.extend(xs)
    track_y.extend(ys)

    xs, ys = line_generator(x0=12.0, y0=7.0, x1=15.0, y1=4.0, mode=(1, 2))
    track_x.extend(xs)
    track_y.extend(ys)
    
    xs, ys = line_generator(x0=15.0, y0=4.0, x1=17.0, y1=12.0, mode=(1, 1))
    track_x.extend(xs)
    track_y.extend(ys)   

    xs, ys = line_generator(x0=13.0, y0=18.0, x1=17.0, y1=12.0, mode=(2, 1))
    track_x.extend(xs)
    track_y.extend(ys)
        
    xs, ys = line_generator(x0=1.0, y0=5.0, x1=13.0, y1=18.0, mode=(2, 2))
    track_x.extend(xs)
    track_y.extend(ys)

    xs, ys = line_generator(x0=1.0, y0=5.0, x1=3.0, y1=3.0, mode=(1, 2))
    track_x.extend(xs)
    track_y.extend(ys)
